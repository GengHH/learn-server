...
2.use database_name 进入想要操作的数据库中
3.show tables 查看所有的数据表



6.进入mysql的命令行界面后，使用语句
...
1.# 我这里得到的最新版 nginx 是 v1.6.3-8.el7
$ yum list nginx
2.# 安装 nginx
$ yum install nginx
...
4.# 查询 nginx-1.6.3-9.el7.x86_64 包文件路径
...
5.#使用yun安装nginx后直接输入以下命令即可运行nginx
（如果使用的是编译安装法，则启动方式不同）
$ nginx
...
8#。修改过nginx.conf配置文件后，可以使用以下命令查看文件是否有问题
...
3#.修改配置文件conf/server.xml改为监听80端口，默认编码utf-8，并开启gzip压缩 (这一步不是必须的)
...
4#.启动tomcat(一定进入tomcat的bin目录才执行的)

# sh startup.sh   或者  ./startup.sh

5#.关闭tomcat(一定进入tomcat的bin目录才执行的)
